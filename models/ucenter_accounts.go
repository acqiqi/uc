...
type UcenterAccounts struct {
	...
	SUser       UcenterUsers `gorm:"ForeignKey:SourceCuid" json:"suser"`
...
func UcenterAccountsS() ([]*UcenterAccounts, error) {
	var mb []*UcenterAccounts
	err := db.Model(&UcenterAccounts{}).Where("type = 1 AND is_dz = 1 AND flag = 1").Preload("User").Preload("SUser").Order("id asc").Find(&mb).Error
